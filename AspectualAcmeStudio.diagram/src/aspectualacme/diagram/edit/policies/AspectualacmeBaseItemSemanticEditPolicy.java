...
import aspectualacme.diagram.edit.helpers.AspectualacmeBaseEditHelper;
...
public class AspectualacmeBaseItemSemanticEditPolicy extends SemanticEditPolicy {
		...
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(
						AspectualacmeVisualIDRegistry.getVisualID((View) view));
	...
	private Command getEditHelperCommand(IEditCommandRequest request,
		...
		if (editPolicyCommand != null) {
			...
			request.setParameter(
					AspectualacmeBaseEditHelper.EDIT_POLICY_COMMAND, command);
		}
		IElementType requestContextElementType = getContextElementType(request);
		request.setParameter(AspectualacmeBaseEditHelper.CONTEXT_ELEMENT_TYPE,
				requestContextElementType);
		ICommand command = requestContextElementType.getEditCommand(request);
		request.setParameter(AspectualacmeBaseEditHelper.EDIT_POLICY_COMMAND,
				null);
		request.setParameter(AspectualacmeBaseEditHelper.CONTEXT_ELEMENT_TYPE,
				null);
		if (command != null) {
	...
	private IElementType getContextElementType(IEditCommandRequest request) {
		IElementType requestContextElementType = AspectualacmeElementTypes
	...
	public static LinkConstraints getLinkConstraints() {
		LinkConstraints cached = AspectualacmeDiagramEditorPlugin.getInstance()
				.getLinkConstraints();
		if (cached == null) {
			AspectualacmeDiagramEditorPlugin.getInstance().setLinkConstraints(
		...
		public boolean canExistAttachment_4001(System container,
			...
			try {
				if (source == null) {
					...
					Object sourceVal = AspectualacmeOCLFactory.getExpression(
							6,
							AspectualacmePackage.eINSTANCE
									.getattachableElement(), env).evaluate(
							source,
				...
				if (target == null) {
					...
					Map<String, EClassifier> env = Collections
							.<String, EClassifier> singletonMap(
									"oppositeEnd", AspectualacmePackage.eINSTANCE.getattachableElement()); //$NON-NLS-1$
					Object targetVal = AspectualacmeOCLFactory.getExpression(
							7,
							AspectualacmePackage.eINSTANCE
									.getattachableElement(), env).evaluate(
							target,
