...
public class AspectualacmeCreationWizard extends Wizard implements INewWizard {
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(AspectualacmeDiagramEditorPlugin
	...
	public void addPages() {
		diagramModelFilePage = new AspectualacmeCreationWizardPage(
				"DiagramModelFile", getSelection(), "aspectualacme_diagram_representation"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new AspectualacmeCreationWizardPage(
				"DomainModelFile", getSelection(), "aspectualacme") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".aspectualacme_diagram_representation".length()); //$NON-NLS-1$
					setFileName(AspectualacmeDiagramEditorUtil
							.getUniqueFileName(getContainerFullPath(),
									fileName, "aspectualacme")); //$NON-NLS-1$
				}
