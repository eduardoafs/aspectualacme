...
public class CircleBorderItemLocator extends BorderItemLocator {

	private static final int halfSizeOfAfffixedChild = 6;
	
	public CircleBorderItemLocator(IFigure parentFigure) {
		super(parentFigure);
		setBorderItemOffset(new Dimension(halfSizeOfAfffixedChild,
				halfSizeOfAfffixedChild));
	}

	public CircleBorderItemLocator(IFigure mainFigure, int preferredSide) {
	 ...
	 * the side for SOUTH or call super)
	 * 
	 * @param side
	...
	protected Point getPreferredLocation(int side, IFigure borderItem) {
		if (side == PositionConstants.SOUTH) {
			...
			int y = parentFigureY;
			/*
			 * Play with this for modify the horizontal position on the activity
			 * Figure
			 */
